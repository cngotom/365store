...
//-- 储存详细信息
/*------------------------------------------------------ */
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'save_desc')
{
    $data  = trim($_POST['data']);
    if(!empty($data))
    {
        $sql = "update ".$ecs->table('goods')." set goods_desc = '".$data."' where goods_id = $goods_id ";
        $db->query($sql);

        $message ="goods_id : $goods_id , data : "."\t".$data;
        $res = 0;
        GLog::Log("365manage_goods", $message);
     
    }
    else
    {
        $message = "goods_id : $goods_id , \tdata : ".$data."传递的为空内容 ";
        
        GLog::Error("365manage_goods", $message);
        $res = 1;
    }
...
if (1  || !$smarty->is_cached('goods.dwt', $cache_id)) //取消缓存
{
    ...
    else
    {
        ...
        create_front_html_editor('goods_desc', $goods['goods_desc'],"desc");
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'editpic'){
	$smarty->display('goods_edit_pic.dwt');
}else{
	$smarty->display('g_edit_goods.dwt');
}
