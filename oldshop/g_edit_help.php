...
$_REQUEST['id'] = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 13;
...
//-- 储存详细信息
/*------------------------------------------------------ */
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'save_article')
{
    $data  = trim($_POST['data']);
    if(!empty($data))
    {
        $sql = "update ".$ecs->table('article')." set content = '".$data."' where article_id = $article_id ";
        $db->query($sql);

        $message ="article_id : $article_id , data : "."\t".$data;
        $res = 0;
        GLog::Log("365manage_goods", $message);
    }
    else
    {
        $message = "article_id : $article_id , \tdata : ".$data."传递的为空内容 ";
...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    $smarty->assign('username',         GLogin::name());
    $smarty->assign('email',            GLogin::email());
    ...
    $sql = "SELECT cat_name FROM " .$ecs->table('article_cat').' WHERE cat_id = '.$article['cat_id'];

    $smarty->assign('article',      $article);
    $smarty->assign('catagory_name',$db->getOne($sql));
...
{
    $smarty->display('g_edit_help.dwt', $cache_id);
}
else
{
    $smarty->display('g_edit_help.dwt', $cache_id);
