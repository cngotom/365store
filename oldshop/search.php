...
else
{
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $keywords  .= "(g.goods_name LIKE '%$val%' OR g.goods_sn LIKE '%$val%' OR keywords LIKE '%$val%' $sc_dsad)";
            ...
            $sql = 'select distinct goods_id from ' . $ecs->table('goods').' g left join '.$ecs->table('category').' c on g.cat_id = c.cat_id' . " WHERE  c.cat_name  LIKE '%$val%' ";
            $res = $db->query($sql);
            while ($row = $db->FetchRow($res))
            {
                $goods_ids[] = $row['goods_id'];
   ...
   // $sort = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, count(com.id_value) as comment_count , IFNULL(AVG(com.comment_rank), 0) AS comment_rank, count(ord.goods_id) as salesnum,g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
