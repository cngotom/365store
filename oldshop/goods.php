...
if(GLogin::isEditor())
{
    header('Location:g_edit_goods.php?id='.$goods_id);
}
/*------------------------------------------------------ */
//-- 改变属性、数量时重新计算商品价格
/*------------------------------------------------------ */
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $smarty->assign('cat_categories',         get_categories_tree($goods['cat_id']));  // 分类树
        ...
        $pro_chandi = $properties['pro']['产地']; if(is_array($pro_chandi)) $pro_chandi = $pro_chandi[0];
        $pro_guige = $properties['pro']['规格']; if(is_array($pro_guige)) $pro_guige = $pro_guige[0];
        $pro_danwei = $properties['pro']['单位']; if(is_array($pro_danwei)) $pro_danwei = $pro_danwei[0];
        $pro_baozhi = $properties['pro']['保质期']; if(is_array($pro_baozhi)) $pro_baozhi = $pro_baozhi[0];
        
        if(!empty($pro_guige)   )
            $smarty->assign('pro_guige',$pro_guige .'/'.$pro_danwei);
        $smarty->assign('pro_chandi',$pro_chandi );
        $smarty->assign('pro_baozhi',$pro_baozhi );
        
        
        $smarty->assign('specification',       $properties['spe']);                              // 商品规格
        $attribute_linked = get_same_attribute_goods($properties);
        $smarty->assign('attribute_linked',  $attribute_linked  );           // 相同属性的关联商品
        
        //同产地
         $smarty->assign('same_area',  $attribute_linked[4]); 
         //相同价格的产品
         $smarty->assign('same_price',  get_same_price_goods( $goods['goods_id'],$goods['shop_price'])); 
        ...
        $_pictures = get_goods_gallery($goods_id); 
        $first_img = $_pictures[0]["big_url"];
        $smarty->assign('first_original',  $first_img  );                    // 商品大图
        $smarty->assign('first_goodsimg',  $_pictures[0]["img_url"]  );                    // 商品中图
        $smarty->assign('pictures',          $_pictures);                    // 商品相册
        $smarty->assign('bought_goods',        get_also_bought($goods_id));                      // 购买了该商品的用户还购买了哪些商品
        $smarty->assign('goods_rank',          get_goods_rank($goods_id));                       // 商品的销售排名
    
        $smarty->assign('leaves_child',            'true');  
...
$sql = 'SELECT count(*) ' .
        'FROM ' . $ecs->table('comment') .' where comment_type = 0 and parent_id = 0 and  id_value = ' . $goods_id;
$count = $db->getOne($sql);
$smarty->assign('comment_count',  $count);      
...
function get_linked_goods($goods_id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '".GLogin::discount()."') AS shop_price, ".
...
function get_user_rank_prices($goods_id, $shop_price)
{
    $sql = "SELECT rank_id, IFNULL(mp.user_price, r.discount * $shop_price / 100) AS price, r.rank_name, r.discount " .
            ...
            "WHERE r.show_price = 1 OR r.rank_id = '".GLogin::rank()."'";
    $res = $GLOBALS['db']->query($sql);
...
function get_package_goods_list($goods_id)
{
    ...
    foreach ($res as $tempkey => $value)
    {
        ...
        $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, p.goods_attr, g.goods_sn, g.goods_name, g.market_price, g.goods_thumb, IFNULL(mp.user_price, g.shop_price * '".GLogin::discount()."') AS rank_price
...
function get_same_price_goods($goods_id,$price)
{
    $min_price  =  $price * 0.8;
    $max_price  = $price * 1.25;
    $same_price = array();

    $sql = 'select goods_id, goods_name, goods_thumb, goods_img, market_price,shop_price from '.$GLOBALS['ecs']->table('goods') ." where shop_price > $min_price && shop_price < $max_price && goods_id <> $goods_id limit 0,5"; 
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
