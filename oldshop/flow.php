...
else if($_REQUEST['act'] =='updateTotalFee')
{
    ...
    $order = isset($_SESSION['flow_order']) ?$_SESSION['flow_order']:flow_order_info();
    $consignee = get_consignee(GLogin::id());
    $cart_goods = cart_goods(CART_ALL_GOODS); // 取得商品列表，�?算合�
...
//print_r($consignee);
    ...
    /* 对商品信�?��� */
    $cart_goods = cart_goods(CART_GENERAL_GOODS); // 取得商品列表，�?算合�
  ...
  //  print_r($order);
     ...
     * 计算订单的费�
     */
     $cart_goods = cart_goods( CART_EXCHANGE_GOODS);
...
//    var_dump($order);exit();
   ...
   /* 判断�?���?��体商� */
    ...
    //将提货券设为已使用
    $time = time();
    $sql =  'update '.$ecs->table('delivery_volumes')." set is_used = 1 ,used_time = $time,order_id = $new_order_id"
            .' where volumes_id in ( select parent_id from '.$ecs->table('cart') ." where  session_id = '".SESS_ID."' and  rec_type = ".CART_EXCHANGE_GOODS."  )";
