 ...
 * ECSHOP �?��流程
 * ============================================================================
 * 版权所� 2005-2010 上海商派网络科技有限�?��，并保留所有权利�
...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    /* 检查：如果商品有�?格，而post的数�?��有�?格，把商品的规格属性通过JSON传到前台 */
    ...
    #记录添加信息
    GLog::Log("add_to_cart",'Goodsid '.$goods->goods_id.' GoodsNum '.$goods->number);
    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            if ($user->login($_POST['username'], $_POST['password'],isset($_POST['remember'])))
            {
                ...
                recalculate_price(); // 重新计算�?��车中的商品价�

                /* 检查购物车�?��否有商品 没有商品则跳�?��首页 */
...

{
...

    {
        ...
        /* 查询：棌查该项是否为基本�?以及是否存在配件*/

        /* 此夐配件是指添加商品时附加的并且是设置了优惠价格的配�?此类配件都有parent_idgoods_number�? */
...
elseif ($_REQUEST['step'] == 'drop_to_collect')
{
    if (GLogin::id() > 0)
    {
        ...
        $count = $db->getOne("SELECT goods_id FROM " . $ecs->table('collect_goods') . " WHERE user_id = '".GLogin::id()."' AND goods_id = '$goods_id'");
        if (empty($count))
        {
            $time = gmtime();
...
else
{
    ...
    /* 取得商品列表，计算合计 */
    $cart_goods = get_general_cart_goods();
    $exchange_goods = get_cart_exchange_goods();
    $smarty->assign('goods_list', $cart_goods['goods_list']);
    $smarty->assign('exchange_goods', $exchange_goods['goods_list']);
