...
if ($action == 'print_quotation')
{
    ...
    $user_rank = $db->getAll("SELECT * FROM " .$ecs->table('user_rank') . "WHERE show_price = 1 OR rank_id = '".GLogin::rank()."'");
    $rank_point = 0;
    if (!empty(GLogin::id()))
    {
        $rank_point = $db->getOne("SELECT rank_points FROM " . $ecs->table('users') . " WHERE user_id = '".GLogin::id()."'");
    }
...
function calc_user_rank($rank, $rank_point)
{
    ...
    foreach($rank as $_rank)
    {
        ...
        {
            if (!empty(GLogin::id()) && ($rank_point >= $_rank['min_points']))
            {
                $_tmprank['ext_price'][] = $_rank['rank_name'];
                $_tmprank['ext_rank'][] = $_rank['discount'];
            }
