...
require(ROOT_PATH . '/includes/lib_order_comment.php');
...
$not_login_arr =
array('login','act_login','register','act_register','simple_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer','ajax_login','ajax_register','ajax_card_login');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus','delivery_volumes', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','order_comment','goods_comment','back_order','do_back_order','combine_card','check_combine');
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            foreach ($fields_arr AS $val)
            {
                ...
                if(!empty($_POST[$extend_field_index]))
                {
                    ...
                    $extend_field_str .= " ('" . GLogin::id() . "', '" . $val['id'] . "', '" . $temp_field_content . "'),";
                }
            }
            ...
            {
                $sql = 'UPDATE ' . $ecs->table('users') . " SET `passwd_question`='$sel_question', `passwd_answer`='$passwd_answer'  WHERE `user_id`='" . GLogin::id() . "'";
                $db->query($sql);
            }
            /* 判断是否需要自动发送注册邮件 */
            if ($GLOBALS['_CFG']['member_email_validate'] && $GLOBALS['_CFG']['send_verify_email'])
            {
                send_regiter_hash(GLogin::id());
            }
            //注册成功后的赠送
            do_after_register();
...
elseif ($action == 'login')
{
    ...
    $smarty->assign('back_act', $back_act);
    $login_type  = isset($_REQUEST['type']) ? trim($_REQUEST['type']) : "";
    if($login_type == "card")
        $smarty->display('user_passport2.dwt');
    else
        $smarty->display('user_passport.dwt');
}
...
elseif ($action == 'act_login')
{
    ...
    if ( GLogin::login($username, $password,isset($_POST['remember'])))
    {
...
elseif ($action == 'ajax_card_login')
{
    ...
    $captcha = intval($_CFG['captcha']);
    //不使用验证码
    if ( 0 && ($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
    {
        if (empty($captcha))
        {
            $result['error']   = 1;
    ...
    if (GLogin::card_login($username, $password))
    {
        $smarty->assign('user_info', get_user_info());
        $ucdata = empty($user->ucdata)? "" : $user->ucdata;
        $result['ucdata'] = $ucdata;
...
/* 处理 简单注册的请求 */
elseif ($action == 'simple_register')
{
    include_once(ROOT_PATH . 'includes/lib_passport.php');
    if (!register(trim($_POST['username']), trim($_POST['password1']), trim($_POST['email'])))
    {
...
elseif ($action == 'profile')
{
    ...
    $smarty->display('user_center/user_profile.dwt');
}
...
elseif ($action == 'check_answer')
{
    ...
    {
     //   GLogin::setName(= $_SESSION['temp_user'];
        throw new Exception("not complete");
        GLogin::setName( $_SESSION['temp_user_name'] );
        unset($_SESSION['temp_user']);
        unset($_SESSION['temp_user_name']);
        $smarty->assign('uid',    GLogin::id());
        $smarty->assign('action', 'reset_password');
        $smarty->display('user_passport.dwt');
    }
}
...
//     $user_name = !empty($_POST['username']) ? trim($_POST['username']) : '';
    ...
    // //用户名和邮件地址是否匹配
    $user_info = $user->get_profile_by_email($email);

   //不检测用户名和邮件的对应关系 直接发邮件
    if ( $user_info)
    {
...
elseif ($action == 'act_edit_password')
{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || (GLogin::id()>0 && GLogin::id() == $user_id && $user->check_user(GLogin::name(), $old_password)))
    {
		
        if ($user->edit_user(array('username'=> (empty($code) ? GLogin::name() : $user_info['user_name']), 'old_password'=>$old_password, 'password'=>$new_password), empty($code) ? 0 : 1))
        {
			$sql="UPDATE ".$ecs->table('users'). "SET `ec_salt`='0' WHERE user_id= '".GLogin::id()."'";
			$db->query($sql);
            $user->logout();
...
elseif ($action == 'act_add_bonus')
{
    ...
    $bouns_sn = isset($_POST['bonus_sn']) ? ($_POST['bonus_sn']) : '';

    if ((is_numeric($bouns_sn)) && add_bonus($user_id, $bouns_sn))
    {
...
elseif ($action == 'order_list')
{
    ...
    //获取时间参数，默认为0代表全部时间内
    $time_type =  isset($_REQUEST['time_type']) ? intval($_REQUEST['time_type']) : 0;
    
    $where = "";
    
    
    //订单状态查询
    $order_status_query = array(  "",  //全部订单
                                        " and shipping_status =  ".SS_SHIPPED,//待确认订单
                                        " and order_status = ".OS_CANCELED, //已取消订单
                                        " and shipping_status = ".SS_RECEIVED,//待评价订单
                                        " and shipping_status =".SS_COMMENT_DONE ,//已完成订单
                                        " and pay_status  =".PS_UNPAYED, //待付款
                                        " and order_status =".OS_BACK_ING ,//已完成订单
                                        " and order_status =".OS_BACK_DONE ,//已完成订单
                                    );
    if($order_status >=0 && $order_status  <   count($order_status_query))
      $where .= $order_status_query[$order_status];
    
    //时间查询
    $one_mounth_ago = time() - 60*60*24*30;
    if($time_type == 1)
    {
        
        $where = $where .' and add_time > '.$one_mounth_ago.' ';
    }
    else if ($time_type == 2){
          $where = $where .' and add_time < '.$one_mounth_ago.' ';
    }
    ...
    $pager  = get_pager('user.php', array('act' => $action,'time_type'=>$time_type,'order_status' =>$order_status), $record_count, $page);

    $orders = get_user_orders($user_id, $pager['size'], $pager['start'],$where);
    //统计信息
    if( $order_status == 0) {
        $statics = get_user_orders_statics($user_id);
  ...
  //  $smarty->assign('merge',  $merge);
   ...
   // print_r($orders);exit();
    $smarty->display('user_center/user_order_list.dwt');
}
/* 商品评价 */
elseif ($action == 'goods_comment')
{
    $doPost = isset($_REQUEST['doPost']) ;
    $goods_id = $_REQUEST['goods_id'];
    $user_name = Glogin::name();
    //处理Post过来的数据
    if($doPost){
        ...
        $cmt = array (
         ...
         $message = add_comment($cmt);
         show_message($message, '返回用户中心', 'user.php', 'info',true);
    }
    else{
    ...
    $smarty->display('user_center/user_goods_comment.dwt');
...
elseif ($action == 'order_comment')
{
     ...
     //是否被评价  
     $is_order_commented =  $db->getOne('select count(*) from '.$ecs->table('order_comment'). " WHERE user_id = $user_id  and order_id = $order_id");
     //获取问题
     ...
     if($doPost){
         ...
         if( $is_order_commented )
         {
             ...
             //储存评论
             $marks =serialize( $_REQUEST['radio_option']);
             $text = $_REQUEST['comment_text'];
             insert_order_comment($user_id,$order_id,$marks,$text);
             //重新刷新
             header('Location:user.php?act=order_comment&order_id='.$order_id);
     ...
     //显示订单评论页面
         ...
         //获取商品
          $goods_list = get_order_comment_goods($user_id,$order_id);
         ...
         //已经评论过
          if($is_order_commented)
          {
                $smarty->assign('is_order_commented',true);
          }
          //还未评论
          else
          {
                ...
                $smarty->assign('is_order_commented',false);
          }
    ...
    $smarty->assign('order_id',$order_id);
     $smarty->display('user_center/user_order_comment.dwt');
}
/* 查看订单详情 */
...
elseif ($action == 'back_order')
{
    ...
    $order = get_order_detail($order_id, $user_id);
    $return_money  = ( $order['money_paid'] + $order['surplus'] ) * 0.95;
    ...
    foreach($banks_map as $key => $val)
    {
            $bank_sel_value .=  "<option value='$key'>$val        </option>";
    }
    
    $smarty->assign('order_id', $order_id);
    $smarty->assign('return_money', $return_money);
    $smarty->assign('bank_sel_value', $bank_sel_value);
    $smarty->display('user_center/user_back_order.dwt');
}
/* 退款后台处理 */
elseif ($action == 'do_back_order')
{
    include_once(ROOT_PATH . 'includes/lib_order.php');
    $order_id = isset($_POST['order_id']) ? intval($_POST['order_id']) : 0;
    $back_type = $_POST['where'];
    $error = "";
    /* 订单详情 */
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    $order = get_order_detail($order_id, $user_id);
    if(!$order)
    {
          show_message('非法参数提交，请您联系管理员');
    }
    else if( $order['order_status'] != OS_CONFIRMED)
    {
        show_message('当前订单状态出错，请您联系管理员');
    ...
    //退款的银行账号
    else if($back_type == 'tobank')
    {

        //退款单
        $back_order  = new BackBank();    
        $name = addslashes( $_POST['name'] );
        $card_no = addslashes($_POST['card_no'] );
        $bank_no = addslashes($_POST['bank_no'] );
        $location =addslashes( $_POST['location'] );
        $netpot = addslashes($_POST['netpot'] );
        ...
        $back_order->name =     $name;             
        $back_order->card_no =  $card_no;
        $back_order->bank_no =  $bank_no;
        $back_order->location  = $location;
        $back_order->netpot = $netpot;
        $back_order->reason = $reason;
        $back_order->mobile = $mobile;
        $back_order->create_time    = $create_time;
        $back_order->status    = 0; //退款申请中
        $back_order->user_id = $user_id;
        $back_order->order_id = $order_id;
        $back_order->money = $order['money_paid'] + $order['surplus'];
    
          //插入数据库       
        if(!$back_order->save())
        {
            $message = "退款到银行失败 。".print_r($back_order,true);
            GLog::error("order_back_bank", $message);
                
              show_message('您的输入有误，请您联系客服进行退款','订单列表', 'user.php?act=order_list');
        }
        else{
             //修改订单状态
            $arr = array(
                'order_status'  => OS_BACK_ING,
            );
            update_order($order_id, $arr);
            
            $message = "用户 ".GLogin::id() . "退款到银行:  成功。";
            GLog::log("order_back_bank", $message);
            
            show_message('您的退款申请已经提交，客服会在1-3个工作日完成退款','订单列表', 'user.php?act=order_list');
        }
...
elseif ($action == 'address_list')
{
    ...
    /* 获得用户所有的收货人信息 */
    $consignee_list = get_consignee_list(GLogin::id());

    if (count($consignee_list) < 5 && GLogin::id() > 0)
    {
        $glogin_email  = GLogin::email();
        /* 如果用户收货人信息的总数小于5 则增加一个新的收货人信息 */
        $consignee_list[] = array('country' => $_CFG['shop_country'], 'email' => isset($glogin_email) ? $glogin_email : '');
    }
...
elseif ($action == 'act_edit_address')
{
    ...
    $address = array(
        'user_id'    => $user_id,
        'address_name' => get_address_name(intval($_POST['country']),intval($_POST['province']),intval($_POST['city']),intval($_POST['district'])). trim($_POST['address']),
...
elseif ($action == 'message_list')
{
    ...
    {
        $sql = "SELECT COUNT(*) FROM " .$ecs->table('feedback').
           " WHERE parent_id = 0 AND user_id = '$user_id' AND user_name = '" . GLogin::name() . "' AND order_id=0";
    }
    ...
    $smarty->assign('message_list', get_message_list($user_id, GLogin::name(), $pager['size'], $pager['start'], $order_id));
    $smarty->assign('pager',        $pager);
...
elseif ($action == 'comment_list')
{
    ...
    $order_comment_list = get_order_comment_list( Glogin::id() );
    $smarty->assign('order_comment_list',$order_comment_list);
    $smarty->display('user_center/user_comment_list.dwt');
}
...
elseif ($action == 'collect')
{
    ...
    else
    {
        ...
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('collect_goods') .
            " WHERE user_id='".GLogin::id()."' AND goods_id = '$goods_id'";
        ...
        else
        {
            ...
            $sql = "INSERT INTO " .$GLOBALS['ecs']->table('collect_goods'). " (user_id, goods_id, add_time)" .
                    "VALUES ('".GLogin::id()."', '$goods_id', '$time')";
...
/* 我的提货卷 */
elseif ($action == 'delivery_volumes')
{
    include_once(ROOT_PATH .'includes/lib_transaction.php');

    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;

    $record_count =  DeliveryVolumes::model()->count('user_id = '.$user_id);

    $pager = get_pager('user.php', array('act' => $action), $record_count, $page);
    try{
        $criteria = new CDbCriteria(); 
        $criteria->addCondition("user_id=$user_id"); 
        $criteria->limit = $pager['size']; 
        $criteria->offset =$pager['start']; 
        $criteria->order = 'create_time desc';
        $volumes = DeliveryVolumes::model()->with('Goods')->findAll($criteria);
        //获取已经被提货的货物
        $sql = 'select parent_id from '.$ecs->table('cart')." where session_id = '".SESS_ID."' and rec_type = ".CART_EXCHANGE_GOODS;
        $res = $db->getAll($sql);
        $used_map = array();
        foreach($res as $r)
        {
            $used_map[$r['parent_id']] = true;
        }
    }
    catch (Exception $e){
        print($e);exit();
    ...
    $smarty->assign('volumes', $volumes);
     $smarty->assign('used_map', $used_map);
    $smarty->display('user_transaction.dwt');
}
...
/*购买远程商品*/
elseif ($action == 'buy_rgoods')
{
    
       include_once('includes/lib_transaction.php');
       include_once(ROOT_PATH .'includes/lib_clips.php');
    $short_consignee_list = get_string_consignee_list(GLogin::id());
    $smarty->assign('consignee_list', $short_consignee_list);
    $smarty->assign('exchange_type',     $exchange_type);
    $smarty->assign('action',     $action);
    $smarty->assign('lang',       $_LANG);
    $smarty->assign('info',        get_user_default(GLogin::id()));
    $smarty->assign('helps',            get_shop_help());       // 网店�?��
    $smarty->display('buy_rgoods.dwt');
}
/* 礼卡合并 */
else if ($action =='combine_card')
{
    include_once('includes/lib_transaction.php');
    include_once(ROOT_PATH .'includes/lib_clips.php');
    $short_consignee_list = get_string_consignee_list(GLogin::id());
    ...
    $smarty->assign('helps',            get_shop_help());       // 网店�?��
     ...
     $smarty->display('user_center/combine_card.dwt');
}
/*  检查合并礼卡 */
else if ($action == 'check_combine')
{
    //获取参数
    ...
    if(intval( $_SESSION['password_error'] ) > 5)
    {
         show_message('对不起，卡号密码有误', '重新充值', 'user.php?act=combine_card');
    }
    ...
    if( GLogin::id() ==  $row['user_id'])
       show_message('对不起，您不能输入同一张卡' ,'重新充值', 'user.php?act=combine_card');
    if($money < 0)
        show_message('对不起，您输入的金额有误' ,'重新充值', 'user.php?act=combine_card');
    
    if($money > $row['user_money'])
         show_message('对不起，您的余额不足' ,'重新充值', 'user.php?act=combine_card');
    
    
    //开始充值
    $cur_card = User::model()->findByPk( GLogin::id() );
    ...
    $cur_accountlog->user_id = GLogin::id();
    ...
    $another_accountlog->user_id = $row['user_id'];
    ...
    $another_accountlog->rank_points = 0;
      ...
      $transaction = Yii::app()->db->beginTransaction();
        try{
            if(!$cur_card->save())
            {
                ...
                $message = "用户 ".GLogin::id() . "转账: $money 成功 。";
     ...
     show_message('恭喜您，您的转账已成功' ,'用户中心', 'user.php');
...
function do_after_register()
{
    ...
    if( intval( $GLOBALS['_CFG']['register_point'] ) >0   )
    {
        $point = intval($GLOBALS['_CFG']['register_point']);
        $user = User::model()->findByPk( GLogin::id() );
        $user->pay_points = $point;
      
        //插入到Pay log
        $accountlog = new AccountLog();
        $accountlog->user_id = GLogin::id();
        $accountlog->pay_points = $point;
        $accountlog->change_desc = "注册送的 $point 积分";
        $accountlog->change_type = 2;
        $accountlog->change_time = time();
        $accountlog->user_money = 0;
        ...
        try{
            if(!$user->save())
            {
                $message = "修改积分失败 。".print_r($user,true);
                GLog::error("user_register", $message);
            }
            else{
                $message = "用户 ".GLogin::id() . "修改积分: $point 成功 。";
                GLog::log("user_register", $message);
            }


            if(!$accountlog->save())
            {
                $message = "注册送积分失败 。".print_r($accountlog,true);
                GLog::error("user_register", $message);
            }       
            else{
                $message = "用户 ".GLogin::id() . "注册送积分: $point 成功 。";
                GLog::log("user_register", $message);
            }
            $transaction->commit(); 
       } catch (Exception $e) {   
    ...
    //赠送优惠劵
     if( intval( $GLOBALS['_CFG']['register_bonus'] ) >0   )
     {
         $userBonus = new UserBonus();
 
        //添加优惠劵
        $userBonus->user_id =   GLogin::id();
        $userBonus->bonus_type_id =  intval( $GLOBALS['_CFG']['register_bonus'] ) ;
        $userBonus->save();
