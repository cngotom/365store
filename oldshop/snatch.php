...
if ($_REQUEST['act'] == 'bid')
{
    ...
    /* 检查用户是否已经出同一价格 */
    $sql = 'SELECT COUNT(*) FROM '.$GLOBALS['ecs']->table('snatch_log'). " WHERE snatch_id = '$id' AND user_id = '".GLogin::id()."' AND bid_price = '$price'";
    ...
    /* 检查用户积分是否足够 */
    $sql = 'SELECT pay_points FROM ' .$ecs->table('users'). " WHERE user_id = '" . GLogin::id(). "'";
    ...
    log_account_change(GLogin::id(), 0, 0, 0, 0-$row['cost_points'],sprintf($_LANG['snatch_log'], $row['snatch_name'])); //扣除用户积分
    $sql = 'INSERT INTO ' .$ecs->table('snatch_log'). '(snatch_id, user_id, bid_price, bid_time) VALUES'.
           "('$id', '" .GLogin::id(). "', '" .$price."', " .gmtime(). ")";
    $db->query($sql);
...
function get_myprice($id)
{
    ...
    {
        /* 取得用户所有价格 */
        $sql = 'SELECT bid_price FROM '.$GLOBALS['ecs']->table('snatch_log'). " WHERE snatch_id = '$id' AND user_id = '".GLogin::id()."' ORDER BY bid_time DESC";
        ...
        $sql = 'SELECT pay_points FROM '. $GLOBALS['ecs']->table('users')." WHERE user_id = '".GLogin::id()."'";
