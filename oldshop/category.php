...
if(GLogin::isEditor())
{
    header('Location:g_edit_category.php?'.$_SERVER['REDIRECT_QUERY_STRING']);
}
...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update','comment_count ','comment_rank' ,'salesnum'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
//if ( 1  !$smarty->is_cached('g_category_edit.dwt', $cache_id))

if( !$smarty->is_cached('g_category_edit.dwt', $cache_id) )
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
    ...
    if (!empty($cat))
    {
        ...
        //$smarty->assign('head_ad_id',$cat['head_ad_id']);
        $smarty->assign('head_ad_id',$cat_id);
    }
    else
    {
        /* 如果分类不存在则返回首页 */
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        foreach ($price_grade as $key=>$val)
        {
            ...
            $price_grade[$temp_key]['price_range'] = $price_grade[$temp_key]['start'] . '&nbsp;-&nbsp;' . $price_grade[$temp_key]['end']."元";
    ...
    $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num ".
            ...
            "GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC limit 0,25";
    ...
    $leaves_child = is_leaves_child($cat_id);
    $smarty->assign('leaves_child',            $leaves_child);  
    if($leaves_child)
    {
        $smarty->assign('categories',       get_categories_tree($cat_id)); // 分类树
        $smarty->assign('parent_id',       get_parent_id($cat_id)); //
    }
    else
...
{
     return $GLOBALS['db']->getOne('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
 ...
 *  是否是叶子节点 
 */
function is_leaves_child($cat_id)
{
    $sql =  "select * from ".$GLOBALS['ecs']->table('category')." where parent_id = $cat_id";
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id ,head_ad_id FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, count(com.id_value) as comment_count , IFNULL(AVG(com.comment_rank), 0) AS comment_rank, count(ord.goods_id) as salesnum,  g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '".GLogin::discount()."') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '".GLogin::rank()."' " .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('comment') . ' AS com  on com.id_value = g.goods_id  AND comment_type = 0 AND com.parent_id = 0 AND com.status = 1 ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('order_goods') . ' AS ord  on ord.goods_id = g.goods_id ' .
            "WHERE $where $ext ".
             'group by g.goods_id '.
            "ORDER BY $sort $order";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['comment_rank'] =  round($row['comment_rank']);
