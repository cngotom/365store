...
function get_message_list($user_id, $user_name, $num, $start, $order_id = 0)
{
    ...
    {
        $sql .= " WHERE parent_id = 0 AND user_id = '$user_id' AND user_name = '" . GLogin::name() . "' AND order_id=0 ORDER BY msg_time DESC";
    }
...
function get_goodsinfo($goods_id)
{
    ...
    if (GLogin::id())
    {
        ...
        $sql = "SELECT ua.consignee, ua.email, ua.tel, ua.mobile ".
               ...
               " WHERE u.address_id = ua.address_id AND u.user_id = '".GLogin::id()."'";
...
function add_booking($booking)
{
    $sql = "INSERT INTO " .$GLOBALS['ecs']->table('booking_goods').
            " VALUES ('', '".GLogin::id()."', '$booking[email]', '$booking[linkman]', ".
...
function get_user_default($user_id)
{
    ...
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    $info['username']  = stripslashes(GLogin::name());
...
function add_tag($id, $tag)
{
    ...
    foreach ($arr AS $val)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ". $GLOBALS['ecs']->table("tag").
                " WHERE user_id = '".GLogin::id()."' AND goods_id = '$id' AND tag_words = '$val'";

        if ($GLOBALS['db']->getOne($sql) == 0)
        {
            $sql = "INSERT INTO ".$GLOBALS['ecs']->table("tag")." (user_id, goods_id, tag_words) ".
                    "VALUES ('".GLogin::id()."', '$id', '$val')";
            $GLOBALS['db']->query($sql);
        }
    }
}
...
function get_rank_info()
{
    ...
    if (GLogin::rank())
    {
        $sql = "SELECT rank_name, special_rank FROM " . $ecs->table('user_rank') . " WHERE rank_id = '".GLogin::rank()."'";
        $row = $db->getRow($sql);
        if (empty($row))
        {
            return array();
        ...
        {
            $user_rank = $db->getOne("SELECT rank_points FROM " . $ecs->table('users') . " WHERE user_id = '".GLogin::id()."'");
