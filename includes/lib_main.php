    ...
    if ($filename != 'index')
    {
        ...
        else
        {
            ...
            elseif ('exchange' == $filename)
            {
             ...
             /* 帮助 */
            elseif ('help' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['exchange'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="help.php">' .
                                '帮助中心' . '</a>';
            }
            /* 其他的在这里补充 */
        }
...
function get_shop_help()
{
   ...
   // print_r($sql);var_dump($arr);print_r($arr);exit();


    $arr2 = array(    "购物指南" =>  array( "购物流程" ,"会员介绍", "团购", "常见问题" ,"联系客服" ),
                     "配送方式" =>  array( "上门自提" ,"快递运输", "特快专递（EMS）", "如何送礼" ,"海外购物" ),
                     "支付方式" =>  array( "货到付款" ,"在线支付", "分期付款", "邮局汇款" ,"公司转账" ),
                     "售后服务" =>  array( "退换货政策" ,"价格保护", "退款说明", "返修/退换货" ,"退款申请" ),
                     "特色服务" =>  array( "夺宝岛" ,"DIY装机", "延保服务" )
    );
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = false)
{
    ...
    {
        $link   = empty($links) ? '返回首页': $links;
        $href    = empty($hrefs) ? 'index.php'       : $hrefs;
...
function recalculate_price()
{
    ...
    $sql = 'SELECT c.rec_id, c.goods_id, c.goods_attr_id, g.promote_price, g.promote_start_date, c.goods_number,'.
                "g.promote_end_date, IFNULL(mp.user_price, g.shop_price * '".GLogin::discount()."') AS member_price ".
...
function get_library_number($library, $template = null)
{
    ...
    else
    {
        ...
        $num = isset($static_page_libs[$template][$lib]) ? $static_page_libs[$template][$lib] :  20;
    }
