...
{
    $sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc, pay_config, pay_show, is_cod' .
            ' FROM ' . $GLOBALS['ecs']->table('payment') .
            ' WHERE enabled = 1 ';
    if (!$support_cod)
    {
...
function order_fee($order, $goods, $consignee)
{
    ...
    else if($order['shipping_id'] > 0 && $total['real_goods_count'] == 0){
          $total['shipping_fee']  = 0;
          $total['shipping_insure'] = 0;
    }
    else{
        $total['shipping_fee']  = "error";
        GLog::log("shipping", "order_fee: " .print_r($order,true).print_r($total,true));
    }
    ...
    $total['amount_formated']  = $total['amount'];//price_format($total['amount'], false);
...
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, goods_weight *goods_number as subweight , " .
            "market_price, goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, is_shipping, " .
            "goods_price * goods_number AS subtotal " .
            "FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' " ;
    if($type == CART_GENERAL_GOODS)
            $sql .= "AND rec_type = '$type'";
 ...
 * @return  float   购物车总金额
 */
function cart_weight($include_gift = true, $type = CART_GENERAL_GOODS)
{
    $sql = "SELECT SUM(goods_weight * goods_number) " .
            " FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' " ;
 ...
 * @return  float   购物车总金额
 */
function cart_num($include_gift = true, $type = CART_GENERAL_GOODS)
{
...
function cart_weight_price($type = CART_GENERAL_GOODS)
{
    ...
    /* 获得购物车中非超值礼包商品的总重量 */
    $sql    = 'SELECT SUM(c.goods_weight * c.goods_number) AS weight, ' .
                    'SUM(c.goods_price * c.goods_number) AS amount, ' .
                    'SUM(c.goods_number) AS number '.
                'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c '.
                'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = c.goods_id '.
                "WHERE c.session_id = '" . SESS_ID . "' " .
                " AND g.is_shipping = 0 AND c.extension_code != 'package_buy'";
    $row = $GLOBALS['db']->getRow($sql);
...
function clear_exchange_cart()
{
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID."' and rec_type = ".CART_EXCHANGE_GOODS;
    $GLOBALS['db']->query($sql);
}
/**
 * 获得指定的商品属性
 ...
 * @return  array
 */
function get_cart_exchange_goods()
{
      /* 初始化 */
    $goods_list = array();
    $total = array(
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $total['goods_num'] += $row['goods_number'];
 ...
 * @return  array
 */
function get_general_cart_goods()
{
    /* 初始化 */
    $goods_list = array();
    $total = array(
