...
function get_profile($user_id)
{
   ...
   // GLogin::email() = $row['email'];    //注册SESSION
    ...
    $info['discount']    = GLogin::discount() * 100 . "%";
    $info['email']       = GLogin::email();
    $info['user_name']   = GLogin::name();
 ...
 * @return  array
 */
function get_address_name($con,$pro,$city,$dis)
{
  //  $region_ids = array($con,$pro,$city,$dis);
    $sql = "SELECT  region_name FROM " . $GLOBALS['ecs']->table('region') .
            " WHERE region_id in ($con,$pro,$city,$dis) " ;
...
function get_string_consignee_list($user_id)
{
    $sql = "SELECT address_id,address_name,is_default,consignee,mobile,tel FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '$user_id' LIMIT 5";
    ...
    foreach($res as $row)
    {
        $phone = empty($row['mobile'])?$row['tel']:$row['mobile'];
        $row['address_name'] = "&nbsp;&nbsp;&nbsp;".$row['consignee']."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;手机:".$phone."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;地址:".$row['address_name'];
        $list[] = $row;
...
function get_short_consignee_list($user_id)
{
    $sql = "SELECT address_id,address_name,is_default,consignee FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '$user_id' LIMIT 5";
 ...
 * @return  array
 */
function get_consignee_by_id($user_id,$address_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
 ...
 */
function get_user_orders_statics($user_id)
{
    $arr    = array( 'total' => 0 ,'completed' => 0,'canceled' => 0,'paying' => 0,'confirming'=>0,'commenting'=>0 ,'unhandled' =>0);

    $sql = "SELECT  order_status, shipping_status, pay_status " . " FROM " .$GLOBALS['ecs']->table('order_info') .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            elseif ($row['shipping_status'] == SS_COMMENT_DONE)
            {
                    $arr['completed'] ++;
            }
            else
...
function get_user_orders($user_id, $num = 10, $start = 0,$where = "")
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
                @$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span> </br><span > <a href="user.php?act=order_comment&order_id='.$row['order_id'] .'">  我要评论  </a> </span>';
            }
            elseif ($row['shipping_status'] == OS_COMMENT_DONE)
            {
                @$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span> </br><span >  已评论  </span>';
            }
            else
            {
                if ($row['pay_status'] == PS_UNPAYED)
        ...
        {
             $row['handler'] =  "<a href=\"user.php?act=back_order&order_id=" .$row['order_id']. '">' .退款. '</a>';;
   ...
   $number = count($arr);
    for($i=0 ; $i < $number ; ++$i)
    {
        ...
        $goodsinfoStr = "<div class='img-list'>";
        $order_id = $arr[$i]['order_id'];
        $goods_table = $GLOBALS['ecs']->table('goods') ;
        $order_goods_table = $GLOBALS['ecs']->table('order_goods') ;
        
        $sql = "select og.goods_id, og.goods_name ,g.goods_thumb from $goods_table g ,$order_goods_table og where g.goods_id = og.goods_id and og.order_id =  $order_id";
        
        
        $goods_info_list  = $GLOBALS['db']->getAll($sql);
        foreach($goods_info_list as $goods_info)
        {
            ...
            $thumb_url = get_image_path($goods_id, $goods_thumb, true);
            
            $goodsinfoStr = $goodsinfoStr."<a href='$goods_url' target='_blank'><img title='$goods_name' src='$thumb_url'></a>" ;
        }
        $goodsinfoStr .= '</div>';
        $arr[$i]['goods_info'] = $goodsinfoStr; 
    }
...
function cancel_order($order_id, $user_id = 0)
{
    ...
    if ($GLOBALS['db']->query($sql))
    {
        ...
        if ($order['user_id'] > 0 && $order['bonus_id'] > 0)
        {
         ...
         //退还提货券的使用
        $sql = "UPDATE ".$GLOBALS['ecs']->table('delivery_volumes') ." SET is_used = 0,order_id=0,used_time=''  WHERE order_id = '$order_id' and user_id = $user_id";
 ...
 *  如果新地址，天价地址并设为默认 
 */
function touch_consignee($consignee)
{
     $name = $consignee['consignee'];
     $address_name  = $consignee['address_name'];
     $user_id = $consignee['user_id'];
     
     $sql = "select address_id,consignee,address_name from" . $GLOBALS['ecs']->table('user_address') .
            "WHERE user_id = '$user_id' and  consignee =   '$name' and address_name = '$address_name'";
    ...
    //将该地址设为默认
     $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            " SET address_id = '$address_id'  WHERE user_id = '$user_id'";
...
function save_consignee($consignee, $default=false)
{
    ...
    if ($default)
    {
        /* 保存为用户的默认收货地址 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            " SET address_id = '$consignee[address_id]' WHERE user_id = '".GLogin::id()."'";
...
function get_order_detail($order_id, $user_id = 0)
{
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            $order['pay_online'] = $pay_obj->get_code($order, $payment,true);
        }
    }
    else
    {
...
function return_to_cart($order_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (empty($cart_goods))
        {
            ...
            $return_goods['user_id']    = GLogin::id();
