...
{
     $sql = 'SELECT cat_name FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = $_id";
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $num = 0;
    $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot', 'promote'=>'recommend_promotion','nomal' =>'index_loop_cat');
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods[$idx]['brand_name']   = $row['brand_name'];
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                       sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length'],false) : $row['goods_name'];
        $goods[$idx]['market_price'] = price_format($row['market_price']);
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
                ...
                "IFNULL(mp.user_price, g.shop_price * '".GLogin::discount()."') AS rank_price " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
    ...
    if ($row !== false)
    {
        ...
        $row['goods_src'] = get_image_path($goods_id, $row['original_img'], true);
        $row['goods_big'] = str_replace ( "thumb_img" ,"big_img",get_image_path($goods_id, $row['goods_thumb'], true) );
...
function get_goods_properties($goods_id)
{
    ...
    foreach ($res AS $row)
    {
        ...
        if ($row['attr_type'] == 0 ||$row['attr_type'] == 3 )
        {
            $group = (isset($groups[$row['attr_group']])) ? $groups[$row['attr_group']] : $GLOBALS['_LANG']['goods_attr'];
            $arr['pro'][$row['attr_name']][] = $row['attr_value'];
        }
        else
        {
...
function get_goods_gallery($goods_id)
{
    ...
    foreach($row as $key => $gallery_img)
    {
        ...
        $row[$key]['source_url'] =    get_image_path($goods_id, $gallery_img['img_original'], true, 'gallery');
        $row[$key]['big_url'] =  $first_img = str_replace('source','big',$row[$key]['source_url'] );
    }
    return $row;
}
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = gg.goods_id AND mp.user_rank = '".GLogin::rank()."' ".
