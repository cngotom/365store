...
class integrate
{
    ...
    function get_profile_by_email($email)
    {
        ...
        $sql = "SELECT " . $this->field_id . " AS user_id," . $this->field_name . " AS user_name," .
                    ...
                    $this->field_pass . " AS password ".
               " FROM " . $this->table($this->user_table) .
               " WHERE " .$this->field_email . "='$post_email'";
        $row = $this->db->getRow($sql);
    ...
    function check_email_login($email, $password )
    {
        ...
        $sql = "SELECT " . $this->field_id .", ".$this->field_name.
                " FROM " . $this->table($this->user_table).
                " WHERE " . $this->field_email . "='" . $post_username . "' AND " . $this->field_pass. " ='" . $this->compile_password(array('password'=>$password)) . "'";
    ...
    function set_session ($username='')
    {
        ...
        {
            $sql = "SELECT user_name,user_id, password, email FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username' or email = '$username' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
