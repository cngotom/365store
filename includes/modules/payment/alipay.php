...
//        }
        ...
        //$sign  = substr($sign, 0, -1). ALIPAY_AUTH;
         ...
         //记录用户准备支付
        $message =  $order['order_sn'].":  amount:".$parameter['amount']." method : alipay";
        GLog::log("payment_topay", $message);
        
        
        $code ='https://www.alipay.com/cooperate/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5';
//      $button = '<div style="text-align:center"><input type="button" onclick="window.open(\'https://www.alipay.com/cooperate/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5\')" value="' .$GLOBALS['_LANG']['pay_button']. '" /></div>';
        ...
        if($is_button)
        {
            $code =  '<div style="text-align:center"><input type="button" onclick="window.open(\'https://www.alipay.com/cooperate/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5\')" value="' .$GLOBALS['_LANG']['pay_button']. '" /></div>';
        }
        return $code;
    }
    ...
    function respond()
    {
        ...
        {
            $message =  "Alipay： money in $order_sn does not match $amount";
        ...
        {
            $message = "Alipay: Pay failed because key not right,get_sign:".$_GET['sign'];
            GLog::log("payment_callback", $message);
            return false;
        }
        ...
        {
             $message = "Alipay: Pay OK".print_r($_GET,true);
             GLog::log("payment_callback", $message);
            /* 改变订单状态 */
