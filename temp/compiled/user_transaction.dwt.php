...
?>
      ...
      <h5><span>我的优惠券</span></h5>
      <div class="blank"></div>
       <table width="100%" border="0" cellpadding="5" cellspacing="0" bgcolor="#dddddd">
        <tr>
          <th align="center" bgcolor="#FFFFFF">序列</th>
...
?>
      ...
      <form name="addBouns" action="user.php" method="post" onSubmit="return addBonus()">
        ...
        优惠券序列
          ...
          <input type="submit" class="bnt_blue_1" style="border:none;" value="添加优惠券" />
        </div>
      </form>
    <?php endif; ?>
   
   
    <?php if ($this->_var['action'] == 'delivery_volumes'): ?>
      <script type="text/javascript">
        <?php $_from = $this->_var['lang']['profile_js']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('key', 'item');if (count($_from)):
    foreach ($_from AS $this->_var['key'] => $this->_var['item']):
?>
      ...
      <h5><span> 我的提货券 </span></h5>
      <div class="blank"></div>
       <table width="100%" border="0" cellpadding="5" cellspacing="0" bgcolor="#dddddd">
        <tr>
          <th align="center" bgcolor="#FFFFFF">序列</th>
          <th align="center" bgcolor="#FFFFFF">对应商品</th>
          <th align="center" bgcolor="#FFFFFF">截止日期</th>
          <th align="center" bgcolor="#FFFFFF">状态</th>
           <th align="center" bgcolor="#FFFFFF">操作</th>
        </tr>
        <?php if ($this->_var['volumes']): ?>
        <?php $_from = $this->_var['volumes']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'item');if (count($_from)):
    foreach ($_from AS $this->_var['item']):
?>
        <tr>
          <td align="center" bgcolor="#FFFFFF"><?php echo empty($this->_var['item']['volumes_sn']) ? 'N/A' : $this->_var['item']['volumes_sn']; ?></td>
          <td align="center" bgcolor="#FFFFFF"><?php echo $this->_var['item']['Goods']['goods_name']; ?></td>
          <td align="center" bgcolor="#FFFFFF"><?php echo $this->_var['item']['expireTime']; ?></td>
            <?php if ($this->_var['item']['is_used'] == 1 || $this->_var['used_map'] [ $this->_var['item']['volumes_id'] ]): ?>
            <td align="center" bgcolor="#FFFFFF">已使用</td>
            <td align="center" bgcolor="#FFFFFF"></td>
            <?php else: ?>
            <td align="center" bgcolor="#FFFFFF"> 未使用</td>
            <td align="center"  bgcolor="#FFFFFF"> <a href="javascript:void(0)" onclick="get_exchange_goods(<?php echo $this->_var['item']['volumes_id']; ?>)" >  <font color="red">提货 </font>  </a> </td>
            <?php endif; ?>
        </tr>
        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
        <?php else: ?>
    ...
    <?php endif; ?>
       ...
       <table width="100%" border="0" cellpadding="5" cellspacing="0" class='order_list_table'>
          <tr align="center">
            <th><?php echo $this->_var['lang']['order_number']; ?></td>
            <th ><?php echo $this->_var['lang']['order_addtime']; ?></td>
            <th><?php echo $this->_var['lang']['order_money']; ?></td>
            <th><?php echo $this->_var['lang']['order_status']; ?></td>
            <th><?php echo $this->_var['lang']['handle']; ?></td>
          </tr>
...
?>
          <tr>
            ...
            <td align="center"><font class="f6"><?php echo $this->_var['item']['handler']; ?></font></td>
          </tr>
          <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
          <tr>
              <td colspan="7" class="td-03"><div class="ar ftx-03"> 等待付款订单数：<strong id="noPayCount" class="ftx-01">0</strong>&nbsp;&nbsp;
                  未完成的订单数：<strong id="noFinishCount" class="ftx-01">0</strong>&nbsp;&nbsp;
                  已完成的订单数：<strong id="finishedCount" class="ftx-01">3</strong>&nbsp;&nbsp;
                  已取消订单数：<strong id="cancelCount" class="ftx-01">1</strong>&nbsp;&nbsp;
                  订单总数：<strong id="totalCount" class="ftx-01">4</strong>&nbsp;&nbsp; </div></td>
            </tr>
          </table>
       <div class="blank5"></div>
         <?php echo $this->fetch('library/pages.lbi'); ?>
      ...
      <?php if ($this->_var['action'] == 'track_packages'): ?>
        ...
        <div class="blank"></div>
        <table width="100%" border="0" cellpadding="5" cellspacing="0" bgcolor="#dddddd" id="order_table">
        <tr align="center">
...
?>
                    ...
                    <select name="district" id="selDistricts_<?php echo $this->_var['sn']; ?>" <?php if (! $this->_var['district_list'][$this->_var['sn']]): ?> <?php endif; ?>>
