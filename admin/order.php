...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if($order['money_paid'] == 0 )
        $order['pay_name'] = '余额支付'; 
    else if( $order['formated_total_fee'] > $order['formated_money_paid'])
        $order['pay_name'] .= ' 部分余额支付';
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn,g.goods_weight,g.seller_note 
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            ...
            $arr = explode(';', $row['seller_note']);//以
            
            if($arr[0])
                $row["link"] = "<a href=\" ".$arr[0]."?tracker_u=103126728 \"> 采购地址</a>";
            $goods_list[] = $row;
    }
    ...
    //取得退款银行信息
    if( OS_BACK_ING == $order['order_status'] || OS_BACK_DONE == $order['order_status'] )
    {
            $banks_map = array(
                '101' => '中国工商银行',
                '102' =>  '中国农业银行',
          ...
          $sql = "SELECT * FROM " . $ecs->table('back_bank') . " WHERE order_id = '$order[order_id]'" ;
          $back_bank = $db->getRow($sql);
        
          $back_bank['bank_name'] = $banks_map[$back_bank['bank_no']];
          $back_bank['create_time']     = date($_CFG['time_format'], $back_bank['create_time']);
          if(!empty($back_bank['back_time'] ))
             $back_bank['back_time']     = date($_CFG['time_format'], $back_bank['back_time']);
          //应退的金额为 付款金额 加余额金额
          
          $back_bank['return_money'] = ($back_bank['money'] ) * 0.95;
          $smarty->assign('back_bank', $back_bank);
    }
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    $do_cms = trim($_REQUEST['do_sms']);
    $sms_data =      trim($_REQUEST['sms_data']);
    if(!empty($do_cms))
    {
        include_once(ROOT_PATH . 'includes/lib_sms.php');
        @sendsms($delivery_order['mobile'],   $sms_data  );
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    /* 退货 */
    elseif (isset($_POST['back_bank_ing']))
    {
        //$require_note   = $_CFG['order_return_note'] == 1;
        $order          = order_info($order_id);
        $action         = $_LANG['op_back_bank'];
        $operation      = 'back_bank_ing';
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    //退款
    elseif ('back_bank_ing' == $operation)
    {
         /* 标记订单为“已退款” */
        $arr = array('order_status'     => OS_BACK_DONE,
        );
        update_order($order_id, $arr);
        
        $back_bank_order = $GLOBALS['db']->getRow('select * from '.$GLOBALS['ecs']->table('back_bank').' where order_id ='.$order_id);
        //update back_bank_order
        $arr = array(
                'money_paid' => $back_bank_order['money'] * 0.95 ,
                'back_time' => time(),
         );
 
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('back_bank'),
        ...
        /* 退货用户余额、积分、红包 */
        return_user_surplus_integral_bonus_no_clear($order);
...
/**
 * 退回余额、积分、红包（取消、无效、退货时），不把订单使用余额、积分、红包设为0
 * @param   array   $order  订单信息
 */
function return_user_surplus_integral_bonus_no_clear($order)
{
    /* 处理积分、红包 */
    if ($order['user_id'] > 0 && $order['surplus'] > 0)
    {
...
function operable_list($order)
{
    ...
    if (OS_BACK_ING == $os)
    {
        $list['back_bank_ing'] = true;
    }
    else if(OS_BACK_DONE == $os)
    {
        $list['back_bank_done'] = true;
    }
    else        /* 售后 */
