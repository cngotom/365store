 ...
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls,$img_width=0,$img_height=0,$image=null)
{
    /* 是否处理缩略图 */
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            if ($proc_thumb && gd_version() > 0)
            {
             ...
             if (  $img_width != 0 && $img_width != 0 && $image != null)
            {
                $img_url = $image->make_thumb('../'. $img_url ,$img_width, $img_width);
                if ($img_url === false)
                {
            ...
            $thumb_url = reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb');
                ...
                if ( 1 ) //需要处理大图
                {
                    $big_img  = str_replace('source','big',$img_original);
                    $org_info = @getimagesize('../'.$img_original);
                    
                   
                    if( $org_info[0] >1000 || $org_info[1] >1000 )
                    {
                        //$temp_img = $image->make_thumb('../' . $original_img, $GLOBALS['_CFG']['big_width'],  $GLOBALS['_CFG']['big_height']);
                        $temp_img =$GLOBALS['image']->make_thumb('../' . $img_original, 1000, 1000);
                        if (!copy('../' . $temp_img, '../' . $big_img))
                        {
                            sys_msg('fail to copy file: ' . realpath('../' . $temp_img), 1, array(), false);
                        }
                        else
                        {
                            @unlink('../' . $temp_img);
                        ...
                        $temp_img = $GLOBALS['image']->make_thumb('../' . $img_original, $maxLength, $maxLength);
                        if (!copy('../' . $temp_img, '../' . $big_img))
                        {
...
function reformat_image_name($type, $goods_id, $source_img, $position='')
{
    ...
    switch($type)
    {
        ...
        case 'gallery':
            ...
            break;
       case 'big':
            $img_name = $goods_id . '_B_' . $rand_name;
            break;
        case 'gallery_thumb':
    ...
    elseif ($position == 'big')
    {
        if (move_image_file(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/big_img/'.$img_name.$img_ext))
        {
            return $dir.'/'.$sub_dir.'/big_img/'.$img_name.$img_ext;
        }
    }
    else
    {
