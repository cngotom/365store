...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    /* 创建 html editor */
    create_html_editor('goods_desc', $goods['goods_desc'],"desc");
    create_html_editor('goods_advice', $goods['goods_advice'],"advice");
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    else
    {
        ...
        if ($proc_thumb && isset($_POST['auto_thumb']) && !empty($original_img))
        {
            ...
            if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0)
            {
                $goods_thumb = $image->make_thumb_keep_ratio('../' . $original_img, $GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height']);
                if ($goods_thumb === false)
                {
                    sys_msg($image->error_msg(), 1, array(), false);
                }
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc,goods_advice, add_time, last_update, goods_type, rank_integral, suppliers_id)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" ."$_POST[goods_advice]', '". gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
    ...
    else
    {
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
                ...
                "goods_advice = '$_POST[goods_advice]', " .
                "last_update = '". gmtime() ."', ".
    ...
    //处理源图
      if ( !empty( $_FILES['goods_img']['tmp_name']) ) //需要处理大图
        {
            $big_img  = str_replace('source','big',$original_img);
            ...
            if( $org_info[0] >1000 || $org_info[1] >1000 )
            {
                 ...
                 $temp_img = $image->make_thumb('../' . $original_img, 1000, 1000);
                 if (!copy('../' . $temp_img, '../' . $big_img))
                {
                     sys_msg('fail to copy file: ' . realpath('../' . $original_img), 1, array(), false);
                }
                else
                {
                     @unlink('../' . $temp_img);
                  ...
                  $temp_img = $image->make_thumb('../' . $img, $maxLength, $maxLength);
                  if (!copy('../' . $temp_img, '../' . $big_img))
                {
                     sys_msg('fail to copy file: ' . realpath('../' . $original_img), 1, array(), false);
                }
          ...
          if (intval($_CFG['watermark_place']) > 0 && !empty($GLOBALS['_CFG']['watermark'])) //加水印
           {
                    if ($image->add_watermark('../'.$big_img,'',$GLOBALS['_CFG']['watermark'], $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']) === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
    ...
    if (isset($img))
    {
        ...
        if ( 1 ) //需要处理大图
        {
            $big_img  = str_replace('source','big',$img);
            $org_info = @getimagesize('../'.$img);
            if( $org_info[0] >1000 || $org_info[1] >1000 )
            {
                 //$temp_img = $image->make_thumb('../' . $original_img, $GLOBALS['_CFG']['big_width'],  $GLOBALS['_CFG']['big_height']);
                 $temp_img = $image->make_thumb('../' . $img, 1000, 1000);
                 if (!copy('../' . $temp_img, '../' . $big_img))
                {
                     sys_msg('fail to copy file: ' . realpath('../' . $img), 1, array(), false);
                }
        ...
        if($is_insert)
        {
             ...
             $db->autoExecute($ecs->table('goods_gallery'), $arr, 'INSERT');
 
        }

        else {
            unset($arr['goods_id']);
            $db->autoExecute($ecs->table('goods_gallery'), $arr, 'update','goods_id = '.$goods_id ." order by img_id asc limit 1");
        }
    ...
    /* 处理相册图片 */
    handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file'],$_CFG['image_width'],$_CFG['image_height'],$image);

    //格式化图片
    $goods_array = array($goods_id);
    $goods_str =  ' goods_id in ('.implode(',',$goods_array).')';
    unifor_goods_img($goods_str);
    unifor_gallery_img($goods_str);
...
function unifor_gallery_img($goods_str)
{
...
function unifor_goods_img($goods_str)
{
