...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        //添加对小图片的处理
        if (!empty($_FILES['small_img_file_src']['name']))
        {
                if(!get_file_suffix($_FILES['small_img_file_src']['name'], $allow_suffix))
            {
                sys_msg($_LANG['invalid_type']);
            }
            $name =  "small_".$basename;

            $target = ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;
            if (move_upload_file($_FILES['small_img_file_src']['tmp_name'], $target))
            {
                $smallsrc = DATA_DIR . '/afficheimg/' . $name;
            }
        }
         else
        {
            $smallsrc = $_POST['small_img_src'];
        }
        ...
        // 插入新数据
        array_unshift($flashdb, array('src'=>$src, 'url'=>$_POST['img_url'], 'text'=>$_POST['img_text'] ,'sort'=>$_POST['img_sort'],'small' =>$smallsrc    ));
        // 实现排序
        $flashdb_sort   = array();
        $_flashdb       = array();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    elseif ($_POST['step'] == 2)
    {
     ...
     // 插入新数据
        $flashdb[$id] = array('src'=>$src,'url'=>$_POST['img_url'],'text'=>$_POST['img_text'],'sort'=>$_POST['img_sort'],'small'=>$smallsrc);
...
function get_flash_xml()
{
    ...
    if (file_exists(ROOT_PATH . DATA_DIR . '/flash_data.xml'))
    {
        ...
        // 兼容v2.7.0及以前版本
        if (!preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\ssort="([^"]*)"\ssmall="([^"]*)"/', file_get_contents(ROOT_PATH . DATA_DIR . '/flash_data.xml'), $t, PREG_SET_ORDER))
        {
            preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\ssmall="([^"]*)"/', file_get_contents(ROOT_PATH . DATA_DIR . '/flash_data.xml'), $t, PREG_SET_ORDER);
        }
        ...
        if (!empty($t))
        {
            foreach ($t as $key => $val)
            {
                ...
                $flashdb[] = array('src'=>$val[1],'url'=>$val[2],'text'=>$val[3],'sort'=>$val[4],"small"=>$val[5]);
            }
        }
    }
    return $flashdb;
...
function put_flash_xml($flashdb)
{
    if (!empty($flashdb))
    {
        ...
        foreach ($flashdb as $key => $val)
        {
            $xml .= '<item item_url="' . $val['src'] .'" link="' . $val['url'] . '" text="' . $val['text'] . '" sort="' . $val['sort'] .'" small="' . $val['small'].'"/>';
        }
        $xml .= '</bcaster>';
