...
function savePart_payAndShipping(obj)
{
    ...
    {
        if(g('payType_IdShipmentType').value == "")
        {
            alert("请选择配送方式");
        }
        else
...
function updatelocalTotalFee(result)
{
       ...
       if(result.code==0 && result.totalfee>=0)
...
function changeSurplus(val)
{
  ...
  Ajax.call('flow.php?act=change_surplus', 'surplus=' + val, changeSurplusResponse, 'GET', 'JSON');
}
...
function changeSurplusResponse(obj)
{
  ...
  else
  {
    try
    {
         ...
         document.getElementById("ECS_SURPLUS_NOTICE").innerHTML = obj.notice;
    }
    catch (ex) { }
      updateTotalFee();//orderSelectedResponse(obj.content);
  }
...
function save_invoice_extra(runCode)
{
    ...
    Ajax.call('flow.php?act=saveInvoiceExtra','invoiceExtra='+$.toJSON(invoiceExtra),function(result){ eval(runCode) } , 'POST', 'JSON');
  //  invoice_head  =                      
   // invoice_content = 
  //  invoice_head = 
...
function submit_order()
{
    ...
    if(check_submit())
    {
        var runCode = "self.location = 'flow.php?step=done';";
        save_invoice_extra(runCode);
